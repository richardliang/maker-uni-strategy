...
/// @title Zap contract that uses flashloans to lever the Uniswap V2 DAIUSDC pool on Maker
...
/// - Assume there are no fees for borrowing DAI against Uni V2 LP. Untested for if DAI stability fees are on
contract MakerUniV2LeverageStrategy is StrategyHelper, ProxyPermission {

    /// @notice Levers a Uni V2 DAI USDC position owned using a Euler flash loan. Must be called by DSProxy.
    ...
    /// @notice Delevers a Uni V2 DAI USDC position owned using a Euler flash loan. Must be called by DSProxy.
    /// @dev Calls an external contract to receive the flash loan due to callback flow. DSProxy authorizes that contract temporarily
    function delever(
        uint256 _daiLoanAmount,
    ...
    function leverCallback(FlashLoanParams memory _flashLoanParams) public {
        // Get pooled amounts on Uni V2 
        (uint256 daiReserve, uint256 usdcReserve) = UniswapV2Library.getReserves(
            address(FACTORY),
            address(DAI),
            address(USDC)
        );

        uint256 usdcRequired = UniswapV2Library.quote(_flashLoanParams.daiToDepositOrLiquidityToWithdraw, daiReserve, usdcReserve);

        /// Approve DAI to PSM
        DAI.approve(address(DSS_PSM), type(uint256).max);
        /// Approve tokens to Uni V2
        ...
        /// Add liquidity
        ( , , uint256 balanceOfLiquidityShares) = ROUTER.addLiquidity(
            address(DAI),
            address(USDC),
            _flashLoanParams.daiToDepositOrLiquidityToWithdraw,
            usdcRequired,
            _flashLoanParams.daiToDepositOrLiquidityToWithdraw, // NOTE: Must be exact. Use flashbots to avoid reverts
            usdcRequired, // NOTE: Must be exact. Use flashbots to avoid reverts
            address(this),
            block.timestamp
        );
        ...
        /// Deposit in Adapter
        UNIV2_DAIUSDC_JOIN.join(urn, balanceOfLiquidityShares);
    ...
    function deleverCallback(FlashLoanParams memory _flashLoanParams) public {
        ...
        uint256 frobAmount = convertTo18(address(UNIV2_DAIUSDC_JOIN), _flashLoanParams.daiToDepositOrLiquidityToWithdraw);
        ...
        UNIV2_DAIUSDC_JOIN.exit(address(this), _flashLoanParams.daiToDepositOrLiquidityToWithdraw);

        /// Remove liquidity
        UNIV2_DAIUSDC.approve(address(ROUTER), type(uint256).max);
        ( , uint256 amountUsdc) = ROUTER.removeLiquidity(
            address(DAI),
            address(USDC),
            _flashLoanParams.daiToDepositOrLiquidityToWithdraw,
